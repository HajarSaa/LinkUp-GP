...
import { closeUserMenuModal } from "../../../../API/redux_toolkit/modals/userProfile/userMenuSlice";
import { openUserPanel } from "../../../../API/redux_toolkit/ui/chatPanelSlice";
import socket from "../../../../API/sockets/socketService"; // تأكد إن ده هو الملف الصحيح عندك
...
function UserMenu() {
  ...
  const statusData = userStatuses?.[loggin_user?.user];
  const status = statusData?.status || "offline";
  const custom = statusData?.customStatus;
  ...
  const handleSetAway = () => {
    socket.emit("updateStatus", { status: "away" }, (response) => {
      if (response?.success) {
        console.log("Status set to away.");
      } else {
        console.error("Failed to set status:", response);
      }
    });
    dispatch(closeUserMenuModal());
  };
  ...
  return (
    <div className={styles.dropdown}>
      ...
      <div className={styles.option} onClick={handleSetAway}>
